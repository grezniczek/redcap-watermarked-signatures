...
[![DOI](https://zenodo.org/badge/1303226347.svg)](https://doi.org/10.5281/zenodo.21892700)
...
| **Custom background image** | Blank | Upload a PNG up to 6 MiB. Each side must be 16–4096 pixels and the image may contain at most 12 million pixels. It is normalized before storage to at most 512 pixels per side and 1 MiB. The aspect ratio must leave at least 16 pixels per side after normalization. The custom-image option is unavailable until an image is stored or selected for upload. The current stored image is shown with a thumbnail and filename; new uploads retain their original filename. The stored image is retained even when another background mode is selected. |
